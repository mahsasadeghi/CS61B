...
public class UDGraph
{
  ...
  public UDGraph paths(int length) {
    ...
    UDGraph doubleNewGraph = new UDGraph(vertices);
    int l=2;
    while(l<=length-1){
        for(int i=0; i<vertices; i++){
            for(int j=0; j<vertices; j++){
                if(newGraph.adjMatrix[i][j]){

                    for(int k=0; k<vertices; k++){
                        if(this.adjMatrix[j][k]){
                            doubleNewGraph.addEdge(i,k);
                        }
                    }
                }
            }
        ...
        l++;
        newGraph = doubleNewGraph;
        doubleNewGraph = new UDGraph(vertices);
    }
    return newGraph;
  }
